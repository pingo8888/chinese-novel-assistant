...
interface RichTextContentMenuSeparator {
	kind: "separator";
}

type RichTextContentMenuOption = RichTextContentMenuItem | RichTextContentMenuSeparator;
...
const CONTENT_MENU_ITEMS: RichTextContentMenuOption[] = [
	{
		command: "bold",
...
export function showRichTextContentMenu(args: ShowRichTextContentMenuArgs): boolean {
...
export function applyRichTextCommand(command: RichTextCommand, editorEl: HTMLTextAreaElement): void {
	editorEl.focus();
	switch (command) {
		case "bold":
...
function isSeparatorOption(item: RichTextContentMenuOption): item is RichTextContentMenuSeparator {
	return (item as RichTextContentMenuSeparator).kind === "separator";
}
